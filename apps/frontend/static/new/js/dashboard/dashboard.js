...
$(document).ready(
	function()
	{
       ...
       // $('.tlm-applet-processes-row-menu').live('click', neoAppletProcesses_manejarMenu);
	}
);
...
function loadAppletData()
{
    ...
    $.post("index.php",arrAction,false,
...
{
	$('.tlm-applet-processes-menu').unbind('click');
	$('html').unbind('click', neoAppletProcesses_esconderMenu);
	$('.tlm-applet-processes-menu').hide();
	return false;
}
...
function neoAppletProcesses_manejarMenu(event)
{
	...
	if ($('.tlm-applet-processes-menu').is(':visible')) {
		neoAppletProcesses_esconderMenu();
	} else {
		event.stopPropagation();

		// Operaciones para cerrar menú cuando se hace clic fuera
		$('.tlm-applet-processes-menu').click(function(event) {
			event.stopPropagation();
		});
		$('html').click(neoAppletProcesses_esconderMenu);

		// Se recuerda qué proceso se va a manejar
		$('#tlm_applet_selected_process').val(sProc);
		
		$('#tlm-applet-processes-controles').show();
		$('#tlm-applet-processes-processing').hide();
		
		$('.tlm_applet_process').unbind('click');
		module_name = 'dashboard';
		
		$('.tlm_applet_process').click(function() {
			$('#tlm-applet-processes-controles').hide();
			$('#tlm-applet-processes-processing').show();
			$.post('index.php?menu=' + module_name + '&rawmode=yes', {
				menu:		module_name, 
				rawmode:	'yes',
				action:		$(this).attr('name'),
				process:	$('#tlm_applet_selected_process').val()
			},
		...
		$('.tlm-applet-processes-menu').show();
		$('.tlm-applet-processes-menu').css("position","fixed");
		$('.tlm-applet-processes-menu').position({
			of: $(this),
			my: "right top",
			at: "right bottom",
			offset: "-7 1"
		...
		if (sCurrState == 'OK') {
			$('#tlm_applet_process_stop').show();
			$('#tlm_applet_process_restart').show();
			$('#tlm_applet_process_start').hide();
		}
		if (sCurrState == 'Shutdown') {
			$('#tlm_applet_process_stop').hide();
			$('#tlm_applet_process_restart').hide();
			$('#tlm_applet_process_start').show();
		}
		if(isActivate == '1')
		{
			$('#tlm_applet_process_activate').hide();
			$('#tlm_applet_process_deactivate').show(); 
		}else{
			$('#tlm_applet_process_activate').show();
			$('#tlm_applet_process_deactivate').hide();
		}
	}
}
