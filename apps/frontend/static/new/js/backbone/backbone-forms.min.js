...
var Form = (function() {
  ...
  return Backbone.View.extend({
     ...
     * @param {String[]|Object[]} [options.fieldsets] How to divide the fields up by section. E.g. [{ legend: 'Title', name: 'field1', 'field2'] }]
     ...
     * ['field1', 'field2']
     * { legend: 'Some Fieldset', name: 'field1', 'field2'] }
     *
...
Form.validators = (function() {
  ...
  validators.required_select = function(options) {
    options = _.extend({
      type: 'required_select',
      message: this.errMessages.required_select
    }, options);

    return function required_select(value) {
      options.value = value;
  ...
  validators.letters = function(options) {
    options = _.extend({
      type: 'letters',
      message: this.errMessages.letters,
      regexp: /^([a-zA-Z ñáéíóú]{1,60})$/i,
    }, options);
...
Form.editors = (function() {
    ...
    initialize: function(options) {
      ...
      for (var i in this.validators)
          if(this.validators[i] == 'required') {
               this.$el.attr('asterisk', 'asterisk');
          }
  ...
  editors.Text = editors.Base.extend({
    ...
    events: {
      ...
      'keypress': function(event) {
        ...
        if (attributes['number']) {
         return justNumbers(event.originalEvent);
        }
    ...
    initialize: function(options) {
      ...
      if (schema && schema.number) this.$el.attr('number', schema.number);
  ...
  editors.Checkboxes = editors.Select.extend({
    ...
    _arrayToHtml: function (array) {
      ...
      _.each(array, function(option, index) {
        ...
        if (_.isObject(option)) {
          var val = (option.val || option.val === 0) ? option.val : '';
          itemHtml += ('<input type="checkbox" name="'+self.id+'" value="'+val+'" id="'+self.id+'-'+index+'" /><label for="'+self.id+'-'+index+'"></label>');
          itemHtml += ('<label for="'+self.id+'-'+index+'">'+option.label+'</label>');
        }
        else {
          itemHtml += ('<input type="checkbox" name="'+self.id+'" value="'+option+'" id="'+self.id+'-'+index+'" /><label for="'+self.id+'-'+index+'"></label>');
          itemHtml += ('<label for="'+self.id+'-'+index+'">'+option+'</label>');
        }
        itemHtml += '</li>';
        html.push(itemHtml);
