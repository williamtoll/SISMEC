...
def getFacturaCompraFiltro(filtros):
    object_list = []
    query_var = []
    query = '''SELECT mc.id, mc.fecha_emision, mc.numero_factura, mc.tipo_factura, mc.estado, mc.monto_total, mc.saldo, p.nombres, p.id, oc.id
               FROM movimiento_cabecera mc, proveedor p, orden_compra_cabecera oc
               WHERE mc.proveedor_id = p.id and mc.orden_compra_id = oc.id '''
        ...
        try:
            ...
            for i in cursor.fetchall():
                data = {'id': i[0],
                        ...
                        'saldo': i[6] if i[6] is not None else '-',
                        'nombres': i[7] if i[7] is not None else '-',
                        'proveedor_id': i[8] if i[8] is not None else '-',
                        'orden_compra_id': i[9] if i[9] is not None else '-',
                        }
                object_list.append(data)
